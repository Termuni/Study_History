...
int solution(vector<vector<int>> maps, vector<vector<int>> time_costs, int max_zero_pass) {
    ...
    vector<vector<vector<int>>> min_time(n, vector<vector<int>>(m, vector<int>(max_zero_pass + 1, MAX_INT)));
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (newY >= 0 && newY < n && newX >= 0 && newX < m) {
                ...
                for (int k = 0; k <= max_zero_pass; ++k) {
                    ...
                    {
                        cout << "[x,y] = [" << newX << ',' << newY << "] // before visited : " << min_time[newY][newX][k + used_zeros] << " and It would be " << new_time << '\n';
